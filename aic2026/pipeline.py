...
class AICPipeline:
    ...
    def __init__(self, frame_index: FrameIndex, videos_dir: str | Path, media_info_dir: str | Path | None = None, reranker: MultimodalReranker | None = None, ranking_weights: RankingWeights | None = None):
    ...
    def retrieve(self, query: str, query_embedding: np.ndarray, top_k_frames: int = 200, top_k_videos: int = 100) -> pd.DataFrame:
        ...
        candidates = best[["video_id", "retrieval_score", "multimodal_score", "best_frame_idx", "best_frame_id", "best_pts_time"]].copy()
    ...
    def localize(self, candidate: pd.Series, frame_scorer: FrameScorer | None = None, radius_frames: int = 15, max_decode_frames: int = 64) -> LocalizedEvent:
    ...
    def run(self, query: str, query_embedding: np.ndarray, top_k: int = 100, frame_scorer: FrameScorer | None = None, radius_frames: int = 15) -> pd.DataFrame:
