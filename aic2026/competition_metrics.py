...
def trake_r_score(
    ...
    """AIC2026 TRAKE R-Score with the strict video gate."""
...
def r_at_k(r_scores: Sequence[float], k: int) -> float:
    """Official Top-k R-Score: maximum R-Score among the first k answers."""
    if k <= 0:
...
def final_score(r_scores: Sequence[float], ks: Sequence[int] = KS) -> float:
    """Official AIC2026 Final Score for one query."""
    if not ks:
...
def ranking_profile(r_scores: Sequence[float], ks: Sequence[int] = KS) -> dict[str, float]:
    """Return the official cutoff profile and Final Score."""
